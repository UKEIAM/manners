...
class ConvBlock(torch.nn.Module):
    ...
    Args:
        ...
        stack_mode (str, optional): Specifies whether to use '1D' or '2D' convolutions. Default is '2D'.
    """
    def __init__(self, in_dim: int, out_dim: int, kernel_size: Union[Tuple[int, int], int], padding: Union[Tuple[int, int], int],
                 ...
                 use_activation: bool = True, use_batch_norm: bool = True, use_dropout: bool = True, stack_mode: str = '2D'):
        super().__init__()
        if stack_mode == '2D':
        ...
        self.batch_norm = (torch.nn.BatchNorm2d if stack_mode == '2D' else torch.nn.BatchNorm1d)(num_features=out_dim, affine=True) if use_batch_norm else None
...
class FullyConvAutoEncoder(torch.nn.Module):
    ...
    Args:
        ...
        stack_mode (str, optional): Stack mode, either '1D' or '2D'. If '2D', values and missing mask are stacked on top of each other. 
        If '1D', missing masks are separate channels. Default is '2D'.
    """
    def __init__(self, bottleneck_dim: int, hidden_dims: List[int], time_kernel_sizes: List[int], time_dilation: List[int],
                 time_strides: List[int], dropout: float = 0.0, variational: bool = True, stack_mode: str = '2D') -> None:
        super().__init__()
        ...
        unflattened_shape = (last_conv_out_dim, 1, result_time_dim) if self.stack_mode == '2D' else (last_conv_out_dim, result_time_dim)
        ...
        for idx, kernel_size in enumerate(self.kernel_sizes[:-1]):
            ...
            shared_encoding_modules.append(ConvBlock(in_dim=in_dim, out_dim=out_dim, kernel_size=kernel_size, padding=self.paddings[idx], stride=self.strides[idx], dilation=self.dilations[idx], reverse=False,
                                                      use_activation=True, use_batch_norm=True, use_dropout=(not self.variational), dropout=self.dropout, stack_mode=self.stack_mode))
        ...
        self.mean_block = torch.nn.Sequential(
            ConvBlock(in_dim=self.hidden_dims[-2], out_dim=self.last_conv_out_dim, kernel_size=self.kernel_sizes[-1], padding=self.paddings[-1], stride=self.strides[-1], dilation=self.dilations[-1],
                      reverse=False, use_activation=False, use_batch_norm=False, use_dropout=False, dropout=self.dropout, stack_mode=self.stack_mode),
            torch.nn.Flatten()
        )
        ...
        for idx, kernel_size in reversed(list(enumerate(self.kernel_sizes))):
            ...
            decoding_modules.append(ConvBlock(in_dim=in_dim, out_dim=out_dim, kernel_size=kernel_size, padding=self.paddings[idx], stride=self.strides[idx], dilation=self.dilations[idx], reverse=True,
                                              use_activation=(idx > 0), use_batch_norm=(idx > 0), use_dropout=(idx > 0 and not self.variational), dropout=self.dropout, stack_mode=self.stack_mode))
    ...
    def decode(self, x_btl: torch.Tensor) -> ResultBatch:
        ...
        nof_value_features = nof_disc_features + len(CLASS_FEATURES)
        if self.stack_mode == '2D':
            x_out_missing = x_out[...,1,:]
            x_disc_out = x_out[:,:nof_disc_features,0,:]
            x_embedded_classes = x_out[:,nof_disc_features:,0,:]
        else:
            x_out_missing = x_out[...,nof_value_features:,:]
            x_disc_out = x_out[:,:nof_disc_features,:]
            x_embedded_classes = x_out[:,nof_disc_features:nof_value_features,:]
        ...
        return ResultBatch(x_out_missing, x_disc_out, x_class_out, x_btl)